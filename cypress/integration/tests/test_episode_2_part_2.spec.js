...
context('Run test for Eposide 2 part 1', () => {
   ...
   it('Go to Server home page', () => {
		cy.wait(5000);
		cy.get('#terminus-console-page').get('.console__page__header').get('.nav__main').get('.nav__main__left')
		.find('a[href="/"]')
		.click().then(() => {
			cy.wait(1000);
		})
    ...
    it('Log in to hub', () => {
		cy.wait(5000);
		//cy.login()
		cy.get('#terminus-console-page').get('.console__page__header').get('.nav__main').get('.nav__main__right')
		.find('button').contains('Connect to hub')
		.click().then(() => {
			cy.wait(1000);
			cy.login();
		})
