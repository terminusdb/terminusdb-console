...
export const CommitTraveller = (props) => {
    ...
    useEffect(() => {
        if (props.commit) {
            TerminusClient.WOQL.lib()
                .commits({'Commit ID': props.commit})
                .execute(woqlClient)
                .then((res) => {
    ...
    const extractCommitData = (res) => {
        let commie = {}
        if (res) {
            if (res['Commit ID'] && res['Commit ID']['@value'])
                commie.id = res['Commit ID']['@value']
            if (res['Time'] && res['Time']['@value'])
                commie.time = parseFloat(res['Time']['@value'])
            if (res['Author'] && res['Author']['@value']) commie.author = res['Author']['@value']
            if (res['Message'] && res['Message']['@value'])
                commie.message = res['Message']['@value']
            if (res['Parent ID'] && res['Parent ID']['@value'])
                commie.parent = res['Parent ID']['@value']
    ...
    function getForwardButton() {
        if (commit && commit.child)
            return <FontAwesomeIcon className={COMMIT_TRAVELLER.className} icon={GO_RIGHT} />
        if (isHead)
            return (
                <FontAwesomeIcon
                    title="This is the head of the branch"
                    className={COMMIT_TRAVELLER.inactiveHistoryClassName}
                    icon={HEAD}
                />
            )
        if (commit)
            return (
                <FontAwesomeIcon
                    title="Next is the head of the branch"
                    className={COMMIT_TRAVELLER.className}
                    icon={HEAD}
                />
            )
        return (
    ...
    function getBackButton() {
        if (commit && commit.parent)
            return <FontAwesomeIcon className={COMMIT_TRAVELLER.className} icon={GO_LEFT} />
        if (commit)
            return (
                <FontAwesomeIcon
                    title="This is the first commit in the record"
                    className={COMMIT_TRAVELLER.inactiveHistoryClassName}
                    icon={HEAD}
                />
            )
        return (
            <FontAwesomeIcon className={COMMIT_TRAVELLER.inactiveHistoryClassName} icon={GO_LEFT} />
        )
    }
        ...
        if (commit && commit.parent) {
            return (
                <Col
                    ...
                    className={COMMIT_TRAVELLER.navColClassName}
                    md={1}
                >
                    <span className={COMMIT_TRAVELLER.navClassName}>{getBackButton()}</span>
                </Col>
            )
        } else {
            return (
                <Col className={COMMIT_TRAVELLER.inactiveNavColClassName} md={1}>
                    <span className={COMMIT_TRAVELLER.inactiveNavClassName}>{getBackButton()}</span>
                </Col>
            )
        }
    }
        ...
        if (commit && commit.child) {
            return (
                <Col onClick={handleNextCommit} className={COMMIT_TRAVELLER.navColClassName} md={1}>
                    <span className={COMMIT_TRAVELLER.navClassName}>{getForwardButton()}</span>
                </Col>
            )
