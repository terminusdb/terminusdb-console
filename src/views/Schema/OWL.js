...
import {
    ...
    OWL_INFO_MSG,
    UPDATE_TURTLE_ERROR,
    UPDATE_TURTLE_SUCCESS,
    DEFAULT_TURTLE_UPDATE_MSG,
...
export const OWL = (props) => {
    ...
    useEffect(() => {
        if (filter) {
            setLoading(true)
            woqlClient
                .getTriples(filter.type, filter.id)
                .then((cresults) => {
    ...
    function updateSchema(commit) {
        ...
        let cmsg = commit
            ? commit
            : DEFAULT_TURTLE_UPDATE_MSG + ' [' + props.graph.type + ',' + props.graph.id + ']'
        setLoading(true)
        woqlClient
            .updateTriples(filter.type, filter.id, updatedTurtle, cmsg)
            .then(() => {
                setEdit(false)
                setDataProvider(updatedTurtle)
                setReport({
                    status: TERMINUS_SUCCESS,
                    message: UPDATE_TURTLE_SUCCESS,
                    time: Date.now() - ts,
                })
            })
            .catch((e) => {
